...
class FancoFanLightAccessory extends BaseAccessory {
    static getCategory(Categories) {
        return Categories.FANCOFANLIGHT;
    }
    ...
    _registerCharacteristics(dps) {
        ...
        this.dpLightOn = this._getCustomDP(this.device.context.dpLightOn) || '15';
        this.dpBrightness = this._getCustomDP(this.device.context.dpBrightness) || '16';
        ...
        if (this.useLight) {
            ...
            if (this.useBrightness) {
                characteristicBrightness = serviceLightbulb.getCharacteristic(Characteristic.Brightness)
                    ...
                    .updateValue(dps[this.dpBrightness] / 10)
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty(this.dpBrightness) && characteristicBrightness && characteristicBrightness.value !== changes[this.dpBrightness] / 10) {
                characteristicBrightness.updateValue(changes[this.dpBrightness] / 10);

            }

            this.log.debug('FancoFanLight changed: ' + JSON.stringify(state));
        });
    }
    ...
    setFanOn(value, callback) {
        ...
        if (value == false) {
            ...
            return this.setMultiStateLegacy({ [this.dpFanOn]: value, [this.dpRotationSpeed]: this.fanCurrentSpeed < 1 ? this.fanDefaultSpeed : this.fanCurrentSpeed }, callback);
    ...
    setBrightness(value, callback) {
        const { Characteristic } = this.hap;
        return this.setState(this.dpBrightness, value * 10, callback);
        callback();
    }
}

module.exports = FancoFanLightAccessory;
